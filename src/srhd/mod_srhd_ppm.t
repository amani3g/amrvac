...
module mod_srhd_ppm
  ...
  public :: srhd_ppm_init

contains

  subroutine srhd_ppm_init()
    use mod_physics_ppm

    phys_ppm_flatcd => srhd_ppm_flatcd
    phys_ppm_flatsh => srhd_ppm_flatsh
  end subroutine srhd_ppm_init

!!! DM Modify (Exclude Bfield)

  subroutine srhd_ppm_flatcd(ixI^L,ixO^L,ixL^L,ixR^L,w,d2w,drho,dpressure)
  ! made by Z. MELIANI 14/02/2018
    ...
    if(srhd_energy) then
      call srhd_get_p_mag(w,ixI^L,ixI^L,pmag)
  ...
  ! made by Z. MELIANI 14/02/2018
    ...
    if(srhd_energy) then
      ...
      call srhd_get_p_total(w,ps(saveigrid)%x,ixI^L,ixI^L,ptot)
      ...
      call srhd_get_csound_prim(w,ps(saveigrid)%x,ixI^L,ixO^L,idims,dpressure)
  ...
  end subroutine srhd_ppm_flatsh

end module mod_srhd_ppm
