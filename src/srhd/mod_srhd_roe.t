...
! DM Modify without the BField
  ...
  !
  ! wL,wR,wroe are all interface centered quantities
  subroutine average2(wL,wR,x,ix^L,idim,wroe,cfast,cslow,afast,aslow,csound2,dpressure, &
       rhodv,tmp)
    use mod_global_parameters
    ...
    double precision, dimension(ixG^T)    :: cfast,cslow,afast,aslow,csound2,dpressure, &
         rhodv,tmp

    if (ndir==1) call mpistop("MHD with d=11 is the same as HD")

    !Averaging primitive variables
    wroe(ix^S,rho_)=0.5d0*(wL(ix^S,rho_)+wR(ix^S,rho_))
    do idir=1,ndir
      wroe(ix^S,mom(idir))=0.5d0*(wL(ix^S,mom(idir))/wL(ix^S,rho_)+wR(ix^S,mom(idir))/wR(ix^S,rho_))
      wroe(ix^S,mag(idir))=0.5d0*(wL(ix^S,mag(idir))+wR(ix^S,mag(idir)))
    end do
    ! Use afast and aslow for pressures pL and pR
    call srhd_get_pthermal(wL,x,ixG^LL,ix^L,afast)
    call srhd_get_pthermal(wR,x,ixG^LL,ix^L,aslow)

    if(srhd_energy) then
      wroe(ix^S,e_)=0.5d0*(afast(ix^S)+aslow(ix^S))
      ! dpressure=pR-pL
      dpressure(ix^S)=aslow(ix^S)-afast(ix^S)
    else
      dpressure(ix^S)=aslow(ix^S)-afast(ix^S)
    end if
    ...
    ! get csound**2
    if(srhd_energy) then
      csound2(ix^S)=srhd_gamma*wroe(ix^S,p_)/wroe(ix^S,rho_)
    else
      csound2(ix^S)=srhd_gamma*srhd_adiab*wroe(ix^S,rho_)**(srhd_gamma-1.0d0)
    end if
    ...
    cslow(ix^S)=0.5d0*(cfast(ix^S)-dsqrt(cfast(ix^S)**2-&
    ...
    afast(ix^S)=min(1.0d0,max(afast(ix^S),zero))

    ! alpha_s=dsqrt(1-alpha_f**2)
    aslow(ix^S)=dsqrt(1.0d0-afast(ix^S))
    ...
    if(ndir==2)then
       ...
       elsewhere
          wroe(ix^S,mag(idir))=dsqrt(0.5d0)
          wroe(ix^S,mag(jdir))=dsqrt(0.5d0)
       end where
    endif
  ...
  subroutine srhd_get_eigenjump(wL,wR,wroe,x,ix^L,il,idim,smalla,a,jump,workroe)
    use mod_global_parameters
  ...
  end subroutine srhd_get_eigenjump
  ...
  subroutine geteigenjump2(wL,wR,wroe,x,ix^L,il,idim,smalla,a,jump, &
       cfast,cslow,afast,aslow,csound2,dpressure,rhodv,bdv,bdb,cs2L,cs2R,cs2ca2L,cs2ca2R)
    use mod_global_parameters
    use mod_tvd
    ...
    double precision, intent(in)          :: x(ixG^T,1:ndim)
    double precision, dimension(ixG^T)    :: smalla,a,jump
    double precision, dimension(ixG^T)    :: cfast,cslow,afast,aslow,csound2,dpressure,rhodv
    double precision, dimension(ixG^T)    :: bdv,bdb,rhohL,rhohR
    ...
    if(il==alfvRW_)then
       ...
       bdv(ix^S)=bdv(ix^S)*0.5d0*wroe(ix^S,rho_)**2
       bdb(ix^S)=bdb(ix^S)*0.5d0*sign(wroe(ix^S,rho_),wroe(ix^S,mag(idim)))
    endif

    select case(il)
    case(fastRW_)
       a(ix^S)=wroe(ix^S,mom(idim))+cfast(ix^S)
       jump(ix^S)=0.5d0/csound2(ix^S)*(&
            afast(ix^S)*(+cfast(ix^S)*rhodv(ix^S)+dpressure(ix^S))&
    ...
    case(slowRW_)
       ...
       jump(ix^S)=0.5d0/csound2(ix^S)*(&
            aslow(ix^S)*(+cslow(ix^S)*rhodv(ix^S)+dpressure(ix^S))&
    ...
    case(entroW_)
       ...
       jump(ix^S)=wR(ix^S,rho_)-wL(ix^S,rho_)-dpressure(ix^S)/csound2(ix^S)
    case(diverW_)
       if(divbwave)then
          a(ix^S)=wroe(ix^S,mom(idim))
    ...
    case('harten','powell', 'ratio')
       ...
       case(fastRW_)
          ...
          rhohL(ix^S)=wL(ix^S,xi_)/wL(ix^S,lfac_)
          rhohR(ix^S)=wR(ix^S,xi_)/wR(ix^S,lfac_)
          call srhd_get_csound2(wL,x,ixG^LL,ix^L,rhohL,cs2L)
          call srhd_get_csound2(wR,x,ixG^LL,ix^L,rhohR,cs2R)
          ...
          aL(ix^S)=aL(ix^S) + dsqrt(0.5d0*(cs2ca2L(ix^S) + cs2L(ix^S)))
          aR(ix^S)=aR(ix^S) + dsqrt(0.5d0*(cs2ca2R(ix^S) + cs2R(ix^S)))
       case(fastLW_)
          aL(ix^S)=aL(ix^S) - dsqrt(0.5d0*(cs2ca2L(ix^S) + cs2L(ix^S)))
          aR(ix^S)=aR(ix^S) - dsqrt(0.5d0*(cs2ca2R(ix^S) + cs2R(ix^S)))
       case(slowRW_)
  ...
  subroutine srhd_rtimes(q,w,ix^L,iw,il,idim,rq,workroe)
    use mod_global_parameters
  ...
  subroutine rtimes2(q,wroe,ix^L,iw,il,idim,rq, &
       cfast,cslow,afast,aslow,csound2,dpressure,rhodv,bv,v2a2)
    use mod_global_parameters
    ...
    if(iw == rho_) then
      ...
      if(il==fastRW_)then
        ...
        v2a2(ix^S)=0.5d0*sum(wroe(ix^S,mom(:))**2,dim=ndim+1)+ &
             (2.0d0-srhd_gamma)/(srhd_gamma-1.0d0)*csound2(ix^S)
        ...
        bv(ix^S)=bv(ix^S)*sign(1.0d0,wroe(ix^S,mag(idim)))
      ...
      case(slowLW_)
        rq(ix^S)=q(ix^S)*(-afast(ix^S)*cfast(ix^S)*bv(ix^S)+aslow(ix^S)*&
             (v2a2(ix^S)+cslow(ix^S)*(cslow(ix^S)-wroe(ix^S,mom(idim)))))
      case(entroW_)
        rq(ix^S)= q(ix^S)*0.5d0*sum(wroe(ix^S,mom(:))**2,dim=ndim+1)
      case(diverW_)
        if(divbwave)then
      ...
      if(iw==mom(idim))then
        ...
        case(fastRW_)
          rq(ix^S)=q(ix^S)*(afast(ix^S)*wroe(ix^S,iw)-aslow(ix^S)*&
               cslow(ix^S)*wroe(ix^S,mag(1)-mom(1)+iw)*sign(1.0d0,wroe(ix^S,mag(idim))))
        case(fastLW_)
          rq(ix^S)=q(ix^S)*(afast(ix^S)*wroe(ix^S,iw)+aslow(ix^S)*&
               cslow(ix^S)*wroe(ix^S,mag(1)-mom(1)+iw)*sign(1.0d0,wroe(ix^S,mag(idim))))
        case(slowRW_)
          rq(ix^S)=q(ix^S)*(aslow(ix^S)*wroe(ix^S,iw)+afast(ix^S)*&
               cfast(ix^S)*wroe(ix^S,mag(1)-mom(1)+iw)*sign(1.0d0,wroe(ix^S,mag(idim))))
        case(slowLW_)
          rq(ix^S)=q(ix^S)*(aslow(ix^S)*wroe(ix^S,iw)-afast(ix^S)*&
               cfast(ix^S)*wroe(ix^S,mag(1)-mom(1)+iw)*sign(1.0d0,wroe(ix^S,mag(idim))))
        case(entroW_)
