...
! DM Modify
 ...
 public :: srhd_con2prim

contains
subroutine srhd_con2prim(d,s,tau,b,lfac,xi,ierror)
  ! made by Z. MELIANI 14/02/2018
! This subroutine srhd_con2prim is re-writed by Z. Meliani
...
! ierror=2 : srhd_maxiterationNR reached without convergence
...
if(ssqr<=small_vec2)then
  call srhd_get_h_noflux(d,tau+d-0.5*bsqr,xi) 
  lfac=1.0d0
  return
endif
...
if(bsqr<=small_vec2)then
  call srhd_srhdcon2prim(d,s,tau,lfac,xi,ierror)
  return 
end if ! Hydro
!}
...
lfacmax=1.0d0/dsqrt(1.0d0-srhd_maxspeed**2.0d0)
! starting point for NR on xi
! xi1 is meant to be the lower bound for the bracket to be used in NR
dplus=d+srhd_gamma*small_pressure/(srhd_gamma-1.0d0)
xi1=dplus
...
!=== find new xi1, in the case v2(xi1) > maxvel^2 = (1-srhd_maxdspeed)^2 
! locate xi corresponding to maximal velocity allowed, namely 1-srhd_maxdspeed
niiter=-1
if(vsqr > srhd_maxspeed**2.0d0 )then 
   er1=1.0d0
   xiprev=xi1
LoopVmax:  do ni = 1,srhd_maxiterationNR
      if(ni>srhd_maxiterationNR/2)then
        xi1=half*(xi1+xiprev)
        er1=10.0d0*er1
      endif
      ...
      fv= ((ssqr + (2.0*xi1 + bsqr)*sb2/(xi1**2.0))&
        /((xi1+bsqr)**2.0))-srhd_maxspeed**2.0d0
      ...
      er=dabs(fv/dfv)/xi1
      if((er<srhd_tolerNR*er1).or.(dabs(fv/dfv)<srhd_absaccnr))exit LoopVmax
      niiter=ni
   enddo LoopVmax
endif 
 !=============================================!

if(niiter==srhd_maxiterationNR)then
...
call srhd_funcd(xi1,fl,df,lfacl,d,ssqr,tau,bsqr,sdotb,ierror)
if(ierror /=0)return
...
LoopxiMax : do ni=1,srhd_maxiterationNR
    ! we now compute f(xi2) and lfac(xi2)
    call srhd_funcd(xi2,fh,df,lfach,d,ssqr,tau,bsqr,sdotb,ierror)
    if(ierror /=0)return
...
if(niiter == srhd_maxiterationNR .or. (fh*fl>zero))then
  ! could not find upper bound for NR on xi
  print *,'could not find upper bound for NR on xi'
  print *,'niiter=',niiter,' versus srhd_maxiterationNR=',&
          srhd_maxiterationNR
...
!   print*, 'taking previous values for initial guess, lfac, xi:', lfaci, xii
   ...
   call srhd_funcd(xii,f,df,lfaci,d,ssqr,tau,bsqr,sdotb,ierror)
...
!--- Start iteration ---!
LoopNRRMHD :  do ni=1,srhd_maxiterationNR 
      nit = nit + 1
      if(nit>srhd_maxiterationNR/2)then
        ! mix the last  value for convergence
        xii=half*(xii+xiprev)
        ! relax accuracy requirement
        er1=10.0d0*er1
        ! following avoids decrease of accuracy requirement 
        ! *every* iteration step beyond srhd_maxiterationNR/2
        nit = nit - srhd_maxiterationNR/10
      endif
      call srhd_funcd(xii,f,df,lfaci,d,ssqr,tau,bsqr,sdotb,ierror) 
      if(ierror /=0)return
      ...
      if(f*df==zero) then
         if(f==zero) then
            ...
            if(tests)then
              ...
              !         srhd_gamma*small_pressure/(srhd_gamma-1.0d0))*lfaci
      ...
      er=dabs(f/df)/xii
      if((er<srhd_tolerNR*er1).or.(dabs(f/df)<srhd_absaccnr))then
            call srhd_funcd(xii,f,df,lfaci,d,ssqr,tau,bsqr,sdotb,ierror) 
            tests=(xii>=dplus*lfaci.and.lfaci<=lfacmax)
            if(tests)then
...
if(niiter==srhd_maxiterationNR) then
     ...
     print *,'er=',er,'srhd_tolerNR=',srhd_tolerNR,'er1=',er1,'df=',df,'srhd_absaccnr=',srhd_absaccnr
     print *,'xii=',xii,' f=',f
     print *,'brackets xil=',xil,' and xih=',xih,' with fl fh=',fl,fh
     print *,'lastxiok=',lastxiok,'lastlfacok=',lastlfacok,'lastf=',lastf
     ierror=2
     return
endif ! niiter==srhd_maxiterationNR
...
!=============================================================================
subroutine srhd_srhdcon2prim(d,s,tau,lfac,xi,ierror)
! This subroutine srhd_srhdcon2prim is re-writed by Z. Meliani 14/02/2018
...
! ierror=7 : stop due to srhd_checkNR violation
...
psmall_pressure=dsqrt(sqrs)/srhd_maxspeed-tau-d
...
LoopNR:  do ni=1,srhd_maxiterationNR
     nit = nit + 1
     !============= Controle ~1~=============!
     if(nit>srhd_maxiterationNR/4)then
     ...
     if(xicurrent<small_xi) then
       print*,'!--- srhd/ in -- con2prim --- at SRHD case!'
     ...
     call srhd_get_val_h_dhdp(rho,pcurrent,drhodp,h,dhdp=dhdp)
...
!        s2overcubeG2rh,h,dhdp,ierror)
     ...
     if(pnew == pLabs .and. pcurrent==pnew .and. &
        abs(ff)> srhd_absaccnr .and. sqrs > zero)then
        pnewi=pnew
...
LoopCor:  do ni2=1,2
           ...
           rho=d/lfac
           call srhd_get_val_h_dhdp(rho,pcurrent,drhodp,h)
...
!                                  tau,sqrs,xicurrent,h,ierror)
       ...
       !=========================================!
        ...
        if( Nff*ff>zero)then
           ...
           s   =zero
           pcurrent     = (srhd_gamma-1.0d0)*tau
           xi=tau+d+pcurrent
           lfac = 1.0d0

           if(srhd_checkNR)ierror=7
     ...
     if(((er<srhd_tolerNR*er1).or.(dabs(dp)<srhd_absaccnr))) exit LoopNR
     !===============================================!
     ...
     if((dabs(oldff2-ff) < 1.0d-8 .or. &
        niiter >= srhd_maxiterationNR-srhd_maxiterationNR/20).and.&
           ff * oldff1 < zero    .and.  dabs(ff)>srhd_absaccnr)then
 ...
 Dicho:  do ni3=1,srhd_maxiterationNR
           !===================!
           xicurrent=tau+d+pcurrent
           v=s/xicurrent
           lfac2inv=1.0d0 - sum(v**2.0d0)
       ...
       !== ZM calculation done using the EOS ==!
           rho=d/lfac
           call srhd_get_val_h_dhdp(rho,pnew,drhodp,h)
...
!                                   tau,sqrs,xicurrent,h,ierror)
       ...
       !=== The iteration converge ===!
           if(2.0d0*dabs(pleft-pright)/(pleft+pright)< srhd_absaccnr &
          .or. dabs(ff)<srhd_absaccnr)then
              pnew=pcurrent
          exit LoopNR
            endif
        !==============================!
...
end subroutine srhd_srhdcon2prim
!=============================================================================
subroutine srhd_funcd(xi,F,dF,lfac,d,ssqr,tau,bsqr,sdotb,ierror)
...
if (vsqr<1.0d0) then
   ...
   call srhd_get_val_p_dpdxi(rho,h,drhodxi,dhdxi,p,dpdxi)   
  ...
  ! print *,'Warning: erroneous input to funcd since vsrq=',vsqr,' >=1'
   ...
   ierror =6
   call mpistop('at srhd_funcd')
   return
end if

end subroutine srhd_funcd
!=============================================================================

end module mod_srhd_con2prim
